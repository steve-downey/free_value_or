...
\item \textbf{R1} Unified with \cite{P3413R0} at LEWG's request: Corentin Jabot
  ...
  P3413R0 deferred and which is now C++26.
...
That divides the models. The division is not an accident of specification.
...
stays \tcode{int\&}, instead of becoming the \tcode{const int\&} that is the
common reference of \tcode{int\&\&} and \tcode{int\&}. There is no
moved-from path to reason about under the dangling analysis. However, it
...
call is a bug, and should be reported as one. We propose the
\tcode{static_assert} behavior (specified as \emph{Mandates}).
...
\item the nullable types \tcode{std::optional<T>},
  ...
  Beman \tcode{optional} reference implementation of P2988
  \cite{The_Beman_Project_beman_optional} --- \tcode{std::optional<T\&>};
\item engaged and disengaged;
\item the nullable as lvalue, const lvalue, and rvalue, the rvalue cases
  copy- and move-counted on the engaged path, so that the ownership split is
...
\item for \tcode{or_construct}, construction arities from zero arguments
  ...
  path; and
\item negative-compilation cases for concept violations and for dangling
  rejection.
\end{itemize}
