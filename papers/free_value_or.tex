...
\item \textbf{R1} Unified with \cite{P3413R0} at LEWG's request: Corentin Jabot
  ...
  implementation. The members are specified over \tcode{optional<T\&>}, which
  P3413R0 deferred and which is now C++26, rather than deferred.
  Independently of the unification, an rvalue nullable now moves its payload,
  which splits the models by ownership, and the asks to LEWG are cut to two.
...
\item \textbf{R1} \today\ --- Unified with \cite{P3413R0} following LEWG's
  ...
  are folded into the proposal, wording, and implementation, and specified
  over \tcode{optional<T\&>}, which P3413R0 deferred. Separately: an rvalue
  nullable moves its payload, splitting the models by ownership, and the asks
  to LEWG are cut to two.
