...
\begin{abstract}
  ...
  the member \tcode{std::optional::value_or} could not.
\end{abstract}
...
semantics available generically. But the overlap is now total --- every
use P3413's members serve has a free-function spelling here that works on
more types and, where a type is computed, computes a better one. If this
paper is adopted, there is far less reason to grow the member surface of
\tcode{optional} and \tcode{expected}; the committee should choose the
long-term spelling deliberately rather than adopting both by accident.
