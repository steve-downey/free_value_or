...
\item \textbf{R1} Unified with \cite{P3413R0} at LEWG's request: Corentin Jabot
  ...
  when its dereference expression permits moving. \tcode{reference_or} now
  requires \tcode{borrowed_nullable}, rejects non-reference common-reference
  results, and rejects temporary owning wrappers.
...
template <borrowed_nullable T, class U,
...
\tcode{reference_or} accepts a \tcode{borrowed_nullable}, computes
...
would materialize a temporary. The concept rejects an rvalue wrapper whose
...
constraint; \tcode{reference_or} additionally rejects rvalue wrappers that
do not model \tcode{borrowed_nullable}, non-reference common-reference
results, and every conversion that would bind the result to a temporary.
...
The \tcode{borrowed_nullable} constraint covers the orthogonal danger that
...
prvalues; there is no reference in their result to check.
...
\item negative-compilation cases for concept violations, non-reference
  ...
  owning \tcode{optional}, \tcode{unique_ptr}, and \tcode{shared_ptr}
  wrappers.
\end{itemize}
...
while raw and smart pointers copy their referents. The alternative is to
...
(see Design). The conversion dangling guards are \emph{Mandates}, so a
