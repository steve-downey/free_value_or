...
\tcode{common_type} fails. However, LEWG guidance is sought on whether the
concept should require \tcode{is_object_v} of the referent, rejecting
function pointers at the constraint and not in the return-type computation.

\section{Exposition-only, or a named public concept?}

Whether \tcode{nullable} is an exposition-only constraint on these four
...
constraints in the standard, is exposition-only for this reason. With
\cite{P1255R13} concluded (see the discussion of \tcode{yield_if} below),
this paper is the concept's sole remaining home. However, that cuts both
ways: no cross-paper coordination is needed, and there is only one chance to
name it right.
...
public name can never be demoted or meaningfully tightened. We therefore
specify \tcode{nullable} as exposition-only, with promotion, under whatever
name LEWG prefers, as the poll alternative.
...
This is a simplification, and we seek LEWG guidance on it. The alternative,
computing the dereference type as
...
However, LEWG guidance is sought on whether \tcode{reference_or} should
further mandate that the computed type \emph{is} a reference type, making
the degenerate calls ill-formed and steering them to \tcode{value_or}.
...
\tcode{or_construct<const int\&>(m, 0)} shows; whether \tcode{Ret} should be
constrained to object types is one for LEWG.
...
\item whether \tcode{or_construct}'s explicit \tcode{Ret} is constrained to
  object types, or the caller may name a reference and own the lifetime;
...
namespace std {
  template<class T>
    concept @\exposid{nullable}@ =                          // exposition only
