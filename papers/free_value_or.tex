...
\begin{abstract}
  ...
  until LEWG asked for one. They fix the cost of the alternative and the
  type of the answer, they compose, and they are severable.
\end{abstract}
...
\item \textbf{R1}
  ...
  \item Unified with \cite{P3413R0} at LEWG's request: Corentin Jabot joins as
    ...
    which is now C++26.
  \item An rvalue nullable moves its payload. R0 dereferenced an lvalue
    always, observing and never consuming; the models now split by
    containment, so \tcode{optional} and \tcode{expected} move their
    subobject, and the pointers, \tcode{optional<T\&>} among them, go on
    copying.
  ...
  \item \tcode{or_invoke} forwards its invocable, agreeing with the
    ...
    \tcode{or_construct} gains the engaged-path \emph{Mandates}, and its
    no-dangle guarantee is stated as holding for a defaulted \tcode{Ret}.
  \item The asks to LEWG are cut from nine to one, whether
    \tcode{reference_or} should mandate a reference result. The rest are
    decided in the paper: the dangling guards are \emph{Mandates}, the concept
    stays minimal, the names stand, and the header is \tcode{<optional>}.
...
and \tcode{value_or_else} still return what \tcode{value_or} beside them
returns, and still require the alternative to converge on \tcode{T}. They fix what they set out to fix, on
...
type is a defect in the signature, and no member can fix it --- the return
type of a member shipping since C++17 cannot change, a corrected second
member would have to be written and kept in sync by hand on every class that
has members at all, and a raw pointer has none (see Why free functions,
below).
...
// on optional the return type is remove_cv_t<T>, as its value_or's is
\end{minted}

Both return what the member \tcode{value_or} beside them returns
(\tcode{remove_cv_t<T>} on \tcode{optional}, \tcode{T} on \tcode{expected}),
and neither constructs anything on the engaged path. The sections that follow
give the design of the free half, where the open choices are; the member
...
That divides the models, and the division is the point, because it is not
an accident of specification. \tcode{std::optional} and
\tcode{std::expected} contain their value: it is a subobject, an rvalue of
one is an expiring object whose payload expires with it, and
\tcode{operator*} is ref-qualified to say so. A pointer contains nothing.
\tcode{T*}, \tcode{std::shared_ptr}, and \tcode{std::unique_ptr} reach a
separate object through indirection, and an expiring handle says nothing
about the lifetime of what it reaches. \tcode{unique_ptr} makes the point
sharpest. It does own its referent, ordinarily; but ownership there is a
policy of the deleter, which may return the object to a pool or do nothing
at all, and the referent is not part of the pointer under any deleter. So
\tcode{unique_ptr::operator*} returns \tcode{T\&} whatever the value
category of the pointer, and the free functions follow it. \tcode{T\&} is
the truthful answer for all three, and they go on copying.

\tcode{std::optional<T\&>} settles which property is doing the work. It is
an \tcode{optional}, and it dereferences to \tcode{T\&}, because its
referent is not a subobject either. The split is containment, not
optional-ness and not ownership. Each nullable reports what its own value
category implies about its payload, and every one of them is right.
...
three smaller things: the return type stops depending on the value category
of the nullable; \tcode{reference_or} over an rvalue \tcode{optional<int>}
stays \tcode{int\&} instead of becoming \tcode{const int\&}, which is the
common reference of \tcode{int\&\&} and \tcode{int\&}; and there is no
moved-from path to reason about under the dangling analysis of
\tcode{reference_or}. However, it buys them by making every containing
nullable copy a payload that is going away, which is the wrong answer stated
...
containment split shows up as test output and a change of rule shows up as a
test failure (see Implementation Experience).

\section{\tcode{reference_or}}

\tcode{reference_or} computes
\tcode{common_reference_t<deref_t<T>, U\&\&>} and, when that type
...
steering them to \tcode{value_or}, is the one question this paper leaves
open below.
...
are settled by the member they join. Both return what \tcode{value_or}
returns, \tcode{remove_cv_t<T>} on \tcode{optional} and \tcode{T} on
\tcode{expected}, and require the alternative to converge on it, because a
member named for
...
answer cannot be done that way. For \tcode{std::optional}
...
What each half fixes stays distinct, and the distinction is worth holding on
to in review. The member half fixes the \emph{cost} of the alternative: with
...
return type & \tcode{T}, as \tcode{value_or} & \tcode{common_type} / \tcode{common_reference} \\
...
rvalue nullable moves payload & yes (\tcode{\&\&} overloads) &
  yes, for the containing models (see Design) \\
...
the rvalue cases copy- and move-counted on the engaged path so that the
containment split is pinned rather than assumed; the alternative as lvalue and
rvalue; mixed-type combinations; constant
...
The suite, at this writing 118 test cases over fifteen translation units
and nine negative-compilation units, all green, found no defects in the
...
That is the containment split of Design showing up as test output. Coverage of the
...
One question is genuinely open, and we ask LEWG to answer it before wording
review: whether \tcode{reference_or} should mandate that the computed common
reference \emph{is} a reference type. As specified it silently degrades to a
prvalue when the payload and the alternative are unrelated ---
...
The rest we have decided, and will revisit only if LEWG disagrees. An rvalue
nullable moves its payload, which divides the models along containment:
\tcode{optional} and \tcode{expected} hold their value as a subobject and
move it, while \tcode{T*}, the smart pointers, and \tcode{optional<T\&>}
reach a separate object and go on copying, \tcode{unique_ptr} included,
since its \tcode{operator*} says \tcode{T\&} whatever the deleter. The
alternative is to dereference an lvalue always, so that the functions
observe and never consume; it is simpler to state, and it makes every
containing nullable copy a payload that is going away. We would rather be correct by default, and the
alternative is a one-line change to the dereference type if LEWG prefers it
(see Design). The
...
namespace std {
  template<class T>
    using @\exposid{deref-t}@ = decltype(*declval<T>());      // \expos
...
\pnum
\tcode{\exposid{deref-t}<T>} is the type that \tcode{*m} yields for an
\tcode{m} of type \tcode{T}, with the value category of \tcode{T} carried
through. For an \tcode{optional<U>} it is \tcode{U\&} when \tcode{T} is an
lvalue reference type and \tcode{U\&\&} otherwise, const-qualified as
\tcode{T} is; for a pointer to \tcode{U} it is \tcode{U\&} whatever the
value category of \tcode{T}.
...
\tcode{is_convertible_v<@\exposid{deref-t}@<T>, R>} is \tcode{true}.
...
and \tcode{is_convertible_v<@\exposid{deref-t}@<T>, R>} is \tcode{true}.
...
purely additive: the existing \tcode{value_or} is unchanged. \emph{Effects}
follow P3413. Return types follow the \tcode{value_or} each member sits
beside: on \tcode{optional} that is \tcode{remove_cv_t<T>}, matching the
reference implementation, which tracks LWG4406, and the \emph{Mandates} are
stated on that type; on \tcode{expected} it is \tcode{T}. In both, the
\emph{Mandates} spell the construction requirement as
\tcode{is_constructible_v<X, Args...>} on the returned type, matching the
\emph{Effects}.
...
\begin{codeblock}
template<class... Args> constexpr remove_cv_t<T> value_or_construct(Args&&... args) const&;
template<class... Args> constexpr remove_cv_t<T> value_or_construct(Args&&... args) &&;

template<class U, class... Args>
  constexpr remove_cv_t<T> value_or_construct(initializer_list<U> il, Args&&... args) const&;
template<class U, class... Args>
  constexpr remove_cv_t<T> value_or_construct(initializer_list<U> il, Args&&... args) &&;

template<class F> constexpr remove_cv_t<T> value_or_else(F&& f) const&;
template<class F> constexpr remove_cv_t<T> value_or_else(F&& f) &&;
\end{codeblock}
...
\pnum
\mandates
\tcode{is_convertible_v<const T\&, X>} is \tcode{true} and
\tcode{is_constructible_v<X, Args...>} is \tcode{true}.
\pnum
\effects
Equivalent to:
\begin{codeblock}
...
\pnum
\mandates
\tcode{is_convertible_v<T, X>} is \tcode{true} and
\tcode{is_constructible_v<X, Args...>} is \tcode{true}.
\pnum
\effects
Equivalent to:
...
\end{codeblock}
\pnum
Let \tcode{X} be \tcode{remove_cv_t<T>} and \tcode{U} be \tcode{invoke_result_t<F>}.
\pnum
\mandates
\tcode{is_convertible_v<const T\&, X>} is \tcode{true} and
\tcode{is_convertible_v<U, X>} is \tcode{true}.
\pnum
\effects
Equivalent to:
\begin{codeblock}
...
\item \textbf{R1} \today\ --- Unified with \cite{P3413R0} following LEWG's
  ...
  that const-observability holds, \tcode{reference_or}'s second dangling guard
  is stated where it can fire, \tcode{or_invoke} forwards its invocable, and
  the asks to LEWG are cut from nine to one. See Changes Since Last Version.
