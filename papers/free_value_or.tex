...
The alternative is to compute the dereference type as
\tcode{decltype(*std::forward<T>(m))}, so that the value category of the
nullable reaches its payload. That divides the models, and the division is
worth being exact about, because it is not an accident of specification.
\tcode{std::optional} and \tcode{std::expected} contain their value. An
rvalue of one is an expiring object whose payload expires with it, and
\tcode{operator*} is ref-qualified to say so. A pointer contains nothing.
\tcode{T*}, \tcode{std::shared_ptr}, and \tcode{std::unique_ptr} are
handles; an rvalue handle is an expiring handle, and says nothing about the
lifetime of the referent, which ordinarily outlives it. \tcode{T\&} is the
truthful answer for all three.

\tcode{std::optional<T\&>} settles which property is doing the work. It is
an \tcode{optional}, and it dereferences to \tcode{T\&}, because it does
not own its referent either. The split is ownership, not optional-ness.

So the alternative would not misbehave. Each nullable would report what its
own value category implies about its payload: an expiring
\tcode{optional<T>} would move, an expiring \tcode{unique_ptr<T>} would go
on copying a referent that is not itself expiring, and both would be right.
The case for the rule proposed here is not correctness.

It is cost and statement. Moving makes the return type depend on the value
category of the nullable, where today an lvalue and an rvalue
\tcode{optional<int>} agree; it turns \tcode{reference_or} over an rvalue
\tcode{optional<int>} from \tcode{int\&} into \tcode{const int\&}, since
that is the common reference of \tcode{int\&\&} and \tcode{int\&}; and it
introduces a moved-from path to reason about under the dangling analysis of
\tcode{reference_or}. Against those, the rule proposed here fits in a
sentence: the result is what the lvalue conditional expression would have
produced, for every nullable alike. Performance-sensitive consuming uses
retain the member functions. We do not think that is a decisive argument,
which is why the question is open.

The reference implementation pins the behavior either way: the engaged path
is copy-counted for \tcode{optional}, \tcode{expected}, and both smart
...
the ownership split of Design showing up as test output. Coverage of the
...
\item whether an rvalue nullable should move its payload. As specified it
  ...
  \tcode{or_construct} the same. Moving would divide the models along
  ownership, correctly: the owning \tcode{optional} and \tcode{expected}
  would move, and the handles would go on copying. The cost is a return type
  that depends on the value category of the nullable (see Design).
\end{itemize}
