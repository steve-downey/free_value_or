...
configure beman.free_value_or to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_FREE_VALUE_OR`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_FREE_VALUE_OR" -C 2
```
...
### `BEMAN_FREE_VALUE_OR_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.free_value_or` can use
`find_package(beman.free_value_or)` to locate the library.
