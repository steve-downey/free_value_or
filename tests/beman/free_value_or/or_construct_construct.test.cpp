...
static_assert(std::is_same_v<decltype(fvo::or_construct(std::declval<std::optional<std::string>&>())), std::string>);

// Pack, many args: optional<string> → R = string (arg shape doesn't change R)
static_assert(std::is_same_v<decltype(fvo::or_construct(std::declval<std::optional<std::string>&>(),
...
static_assert(
    std::is_same_v<decltype(fvo::or_construct(
                       std::declval<std::optional<std::pair<int, int>>&>(), std::declval<int>(), std::declval<int>())),
                   std::pair<int, int>>);

// Init-list: optional<vector<int>> → R = vector<int>
static_assert(std::is_same_v<decltype(fvo::or_construct(std::declval<std::optional<std::vector<int>>&>(),
                                                        std::initializer_list<int>{})),
                             std::vector<int>>);

// Explicit Ret with init-list: or_construct<vector<long>>(..., {1L,2L,3L}) → vector<long>
static_assert(std::is_same_v<decltype(fvo::or_construct<std::vector<long>>(
                                 std::declval<std::optional<std::vector<long>>&>(), std::initializer_list<long>{})),
                             std::vector<long>>);
