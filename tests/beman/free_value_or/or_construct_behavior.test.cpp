...
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<std::optional<int>&>(), std::declval<int>())),
    int>);
...
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<const std::optional<int>&>(), std::declval<int>())),
    int>);

// expected<int,int> + int → R = int
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<std::expected<int, int>&>(), std::declval<int>())),
    int>);

// int* + int → R = int
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<int*>(), std::declval<int>())),
    int>);

// shared_ptr<int> + int → R = int
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<std::shared_ptr<int>&>(), std::declval<int>())),
    int>);

// unique_ptr<int> rvalue + int → R = int
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<std::unique_ptr<int>>(), std::declval<int>())),
    int>);
...
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<std::optional<int>&>(), std::declval<long>())),
    int>);

// optional<long> + int → R = long (arg int converts inward to long)
static_assert(std::is_same_v<
    decltype(fvo::or_construct(std::declval<std::optional<long>&>(), std::declval<int>())),
    long>);

// Explicit Ret: or_construct<long>(optional<int>, ...) → R = long
static_assert(std::is_same_v<
    decltype(fvo::or_construct<long>(std::declval<std::optional<int>&>(), std::declval<long>())),
    long>);

// Explicit Ret: or_construct<std::string>(optional<std::string_view>, ...) → R = std::string
static_assert(std::is_same_v<
    decltype(fvo::or_construct<std::string>(std::declval<std::optional<std::string_view>&>(),
...
TEST_CASE("or_construct behavior: value categories of m", "[or_construct]") {
    ...
    SECTION("rvalue engaged") {
        CHECK(fvo::or_construct(std::optional<int>{42}, 0) == 42);
    }
    ...
    SECTION("rvalue disengaged") {
        CHECK(fvo::or_construct(std::optional<int>{}, 99) == 99);
    }
}
...
TEST_CASE("or_construct behavior: explicit Ret as std::string from string_view payload",
          "[or_construct]") {
...
TEST_CASE("or_construct behavior: explicit Ret with multi-arg disengaged construction",
          "[or_construct]") {
...
TEST_CASE("or_construct behavior: explicit Ret is exactly Ret, not common_type", "[or_construct]") {
    ...
    std::optional<int> m{};
    static_assert(std::is_same_v<
        decltype(fvo::or_construct<long>(m, 0L)),
        long>);
}
