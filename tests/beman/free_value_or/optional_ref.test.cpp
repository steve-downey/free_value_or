...
static_assert(
    std::is_same_v<decltype(fvo::value_or(std::declval<fvo_opt::optional<int&>&>(), std::declval<int&>())), int>);

// reference_or with optional<int&> lvalue + int lvalue → int&
static_assert(
    std::is_same_v<decltype(fvo::reference_or(std::declval<fvo_opt::optional<int&>&>(), std::declval<int&>())), int&>);

// reference_or with optional<int&> lvalue + const int lvalue → const int&
static_assert(
    std::is_same_v<decltype(fvo::reference_or(std::declval<fvo_opt::optional<int&>&>(), std::declval<const int&>())),
                   const int&>);
...
TEST_CASE("reference_or: mutation through reference changes fallback when disengaged",
          "[optional_ref][reference_or]") {
    fvo_opt::optional<int&> d;
    int                     fallback = 0;
    int&                    r        = fvo::reference_or(d, fallback);
    r                                = 99;
