...
static_assert(
    std::is_same_v<decltype(smd::free_value_or::reference_or(std::declval<expected_ref>(), std::declval<int&>())),
                   int&>);
...
TEST_CASE("disengaged temporary expected<T&, E> refers to the fallback", "[expected_ref][reference_or]") {
    int  fallback = 7;
    int& result   = smd::free_value_or::reference_or(expected_ref{beman::expected::unexpected<int>{0}}, fallback);
