...
static_assert(
    std::is_same_v<decltype(fvo::reference_or(std::declval<const std::optional<int>&>(), std::declval<const int&>())),
                   const int&>);

// optional<int>& + const int& → common_reference_t<int&, const int&> = const int&
static_assert(
    std::is_same_v<decltype(fvo::reference_or(std::declval<std::optional<int>&>(), std::declval<const int&>())),
                   const int&>);
...
static_assert(std::is_same_v<decltype(fvo::reference_or(std::declval<std::optional<std::string>&>(),
                                                        std::declval<std::string&>())),
                             std::string&>);

// Return is always a reference type (not a prvalue), unlike value_or
static_assert(
    std::is_reference_v<decltype(fvo::reference_or(std::declval<std::optional<int>&>(), std::declval<int&>()))>);

// ==========================================================================
// Case 1 & 2: Reference identity — engaged points into m, disengaged into u
// ==========================================================================

TEST_CASE("reference_or: optional<int> engaged returns reference to contained value", "[reference_or]") {
...
TEST_CASE("reference_or: optional<int> disengaged returns reference to fallback", "[reference_or]") {
    std::optional<int> m{};
    int                fallback = 99;
    int&               r        = fvo::reference_or(m, fallback);
    CHECK(&r == &fallback);
...
TEST_CASE("reference_or: mutation through reference changes contained value (engaged)", "[reference_or]") {
    std::optional<int> m{42};
    int                fallback = 0;
    int&               r        = fvo::reference_or(m, fallback);
    r                           = 99;
...
TEST_CASE("reference_or: mutation through reference changes fallback (disengaged)", "[reference_or]") {
    std::optional<int> m{};
    int                fallback = 0;
    int&               r        = fvo::reference_or(m, fallback);
    r                           = 99;
...
TEST_CASE("reference_or: optional<string> engaged returns reference to string", "[reference_or]") {
...
TEST_CASE("reference_or: optional<string> disengaged returns reference to fallback string", "[reference_or]") {
